...
class FluxGeneratorClass:
    ...
    def __init__(self, Table, LogTable=True, LogAxis=['tau']):
        ...
        '''
        self._Interpolator = InterpolatorClass(
            Table, LogTable=LogTable, LogAxis=LogAxis)
    ...
    def GetTransformedValue(self, Taus, P):
        ...
        Position = np.array(
            [[tau, P['Eta0'], P['GammaB'], P['theta_obs']] for tau in Taus])
        ...
        if np.isnan(f_peak[0]):
            ...
            f1 = (1+P['z'])/(P['dL']*P['dL'])*(P['p']-1) / \
                (3.*P['p']-1.)*P['E']*P['n']**0.5*P['epsb']**0.5*P['xiN']
            f2 = 1./(1+P['z'])*P['E']**(-2./3.) * \
                P['n']**(-5./6.)*P['epsb']**(-3./2.)
            f3 = 1./(1+P['z'])*(P['p']-2.)**2/(P['p']-1.)**2 * \
                P['n']**(1./2.)*P['epse']**2*P['epsb']**(1./2.)*P['xiN']**(-2)
    ...
    def GetSpectral(self, Times, Freqs, P):
        ...
        if np.isnan(F_peak[0]):
            ...
            output[IdxSlow1] = np.power(
                Freqs[IdxSlow1] * inu_m[IdxSlow1], 1.0/3.0)
            output[IdxSlow2] = np.power(
                Freqs[IdxSlow2] * inu_m[IdxSlow2], 0.5-0.5*p)
            output[IdxSlow3] = np.power(
                nu_c[IdxSlow3] * inu_m[IdxSlow3], 0.5-0.5*p) * np.power(Freqs[IdxSlow3] * inu_c[IdxSlow3], -0.5*p)

            output[IdxFast1] = np.power(
                Freqs[IdxFast1] * inu_c[IdxFast1], 1.0/3.0)
            output[IdxFast2] = np.power(
                Freqs[IdxFast2] * inu_c[IdxFast2], -0.5)
            output[IdxFast3] = np.power(
                nu_m[IdxFast3] * inu_c[IdxFast3], -0.5) * np.power(Freqs[IdxFast3] * inu_m[IdxFast3], -0.5*p)
    ...
    def GetIntegratedFlux(self, Times, Freqs, P):
        ...
        if np.isnan(F_peak[0]):
            ...
            output[IdxSlow1] = 0.75 * nu_m[IdxSlow1] * (np.power(np.minimum(
                HFreqs[IdxSlow1], nu_m[IdxSlow1])*inu_m[IdxSlow1], 4.0/3.0) - np.power(LFreqs[IdxSlow1]*inu_m[IdxSlow1], 4.0/3.0))
            output[IdxSlow2] = 2.0/(3.0-p) * nu_m[IdxSlow2] * (np.power(np.minimum(HFreqs[IdxSlow2], nu_c[IdxSlow2]) *
                                                                        inu_m[IdxSlow2], 1.5-0.5*p) - np.power(np.maximum(LFreqs[IdxSlow2], nu_m[IdxSlow2])*inu_m[IdxSlow2], 1.5-0.5*p))
            output[IdxSlow3] = 2.0/(2.0-p) * nu_c[IdxSlow3] * np.power(nu_c[IdxSlow3]*inu_m[IdxSlow3], 0.5-0.5*p) * (np.power(
                HFreqs[IdxSlow3]*inu_c[IdxSlow3], 1.0-0.5*p) - np.power(np.maximum(LFreqs[IdxSlow3], nu_c[IdxSlow3])*inu_c[IdxSlow3], 1.0-0.5*p))

            output[IdxFast1] = 0.75 * nu_c[IdxFast1] * (np.power(np.minimum(
                HFreqs[IdxFast1], nu_c[IdxFast1])*inu_c[IdxFast1], 4.0/3.0) - np.power(LFreqs[IdxFast1]*inu_c[IdxFast1], 4.0/3.0))
            output[IdxFast2] = 2.0 * nu_c[IdxFast2] * (np.sqrt(np.minimum(HFreqs[IdxFast2], nu_m[IdxFast2])*inu_c[IdxFast2]) - np.sqrt(
                np.maximum(LFreqs[IdxFast2], nu_c[IdxFast2])*inu_c[IdxFast2]))
            output[IdxFast3] = 2.0/(2.0-p) * nu_m[IdxFast3] / np.sqrt(nu_m[IdxFast3]*inu_c[IdxFast3]) * (np.power(
                HFreqs[IdxFast3]*inu_m[IdxFast3], 1.0-0.5*p) - np.power(np.maximum(LFreqs[IdxFast3], nu_m[IdxFast3])*inu_m[IdxFast3], 1.0-0.5*p))
